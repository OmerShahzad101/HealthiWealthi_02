...
const Create = () => {
  ...
  const changeSchema = (e, i) => {
    debugger
    let rolesAdd = [...rolesAdded];
    let testRole = [...rolesAdded];

    let name = e.target.value;
    rolesAdd[i].role = name;
    testRole[i].role = name;
    let check = true;
    for (let x = 0; rolesAdd.length > x; x++) {
      for (let y = 0; testRole.length > y; y++) {
        if (x !== y && check === true) {
          if (rolesAdd[x].role === testRole[y].role) {
            setErrorNameMessage("Role Must Be Unique!");
            check = false;
          }
        }
      }
