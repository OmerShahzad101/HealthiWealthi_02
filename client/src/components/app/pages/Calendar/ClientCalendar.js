...
class ClientCalendar extends Component {
    ...
    generateWeekDates = () => {
        setTimeout(() => {
            ...
            while (tempStartDate <= this.state.endDate) {
                ...
                tempStartDate = moment(tempStartDate).add(1, "days").format("MM/DD/YYYY");
            }
    ...
    render() {
        ...
        var selectedHumanReadableDate = moment(this.state.date).format('MMM DD, YYYY');
        ...
        const gridSlots = this.state.dates.map((date, index) => {
            ...
            if (date == this.state.today)
                ...
                gridClass = "disabled-slot";

            return (
                <div className={`gridDay ${gridClass}`} key={date}>
                    <div className="gridDayHeader">
                        <h3>
                            <span className="gridDayHeaderFull" id={`col-header-${index}`} data-content={humanReadableDate}>{humanReadableDate}</span>
                            <span className="gridDayHeaderName" data-content={dayName}>{dayName} </span>
                            {
                                date == this.state.today ? (
                                    <span className="gridDayHeaderToday"> Today</span>
                                ) : null
                            }
                            <span className="gridDayHeaderDate" data-content={day}>{day}</span>
                                ...
                                <div className="gridSlot" key={timeStop}>
                                    {
                                        date < this.state.today ? (
                                            <span>{timeStop}</span>
                                                ...
                                                <Link to="#" onClick={(event) => this.handleOnClickGridSlot(event, timeStop, humanReadableDate)}>{timeStop}</Link>
                                            )
                                    }
                                </div>
                            ))
                            ...
                            <div className="col-md-6 text-right">
                                <div className=" datepick-wrap">
                                    ...
                                    <DatePicker
                                        ...
                                        />
                                    <span className="dp-icon"><i className="fa fa-calendar"></i></span>
                                </div>
                                <div className="week-controls">
                                    <button title="Prev Week" className="btn btn-arrow btn-sm" onClick={this.handleOnClickPrevButton} disabled={this.state.disabledPrevButton}><i className="fa fa-chevron-left" aria-hidden="true"></i></button>
                                    &nbsp;&nbsp;
                                    <button title="Next Week" type="button" className="btn btn-arrow btn-sm" onClick={this.handleOnClickNextButton}><i className="fa fa-chevron-right" aria-hidden="true"></i></button>
                                </div>
                            </div>
                        </div>
