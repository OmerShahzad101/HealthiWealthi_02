...
function SignaturePad (selector, options) {
  ...
  function drawSignature (paths, context, saveOutput) {
    //console.log("drawSignature");
    //window.myContext = context;
    //window.myScale = settings.scale;
  ...
  $.extend(self, {
    ...
    , getSignatureImage : function () {
      ...
      //reapeaterimg
      var img = document.getElementById("reapeaterimg");
  ...
  this.each(function () {
    if (!$.data(this, 'plugin-signaturePad')) {
      ...
      if(pathsMy){
        ...
        setTimeout(function(){api.regenerate(pathsMy);},500);
      }
    } else {
      api = $.data(this, 'plugin-signaturePad')
      api.updateOptions(options);
      if(pathsMy){
        api.regenerate(pathsMy);
      }
    }
  })
  return api
